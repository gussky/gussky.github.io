...
const NutrientComposition: React.FC<{ csvUrl?: string }> = ({ csvUrl = DEFAULT_CSV_URL }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const x = d3.scaleLinear().domain([-15, 40]).range([0, plotWidth]);
      ...
      const zones = [
        ...
        { id: "E", min: 18.5, max: 40, color: "#e63e11", opacity: 0.1, label: "> 19" },
      ];
