...
const foodData = foodDataRaw as FoodPoint[];
...
export default function LinkedD3Dashboard() {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-slate-50/50 font-sans text-gray-900">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                        <button
                            ...
                            className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${interactionMode === 'select' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Box Select
                        </button>
                        <button
                            onClick={() => setInteractionMode('zoom')}
                            className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${interactionMode === 'zoom' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Zoom
                        </button>
                    </div>
                    ...
                    {stats && (
                        <div className="flex gap-6 text-xs items-center border-l border-gray-200 pl-6">
                            <div className="flex flex-col">
                                <span className="text-gray-400 font-bold uppercase text-[10px] tracking-wider">Count</span>
                                <span className="font-bold text-base leading-none">{stats.total}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-gray-400 font-bold uppercase text-[10px] tracking-wider">Top Cat</span>
                                <span className="font-bold text-base leading-none">{stats.topCategory}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-gray-400 font-bold uppercase text-[10px] tracking-wider">Ultra-Processed</span>
                                <span className={`font-bold text-base leading-none ${stats.ultraProcessedPct > 50 ? 'text-rose-500' : 'text-emerald-500'}`}>
            ...
            <div className="flex-grow flex p-6 gap-6 items-center justify-center relative overflow-hidden">
                ...
                <div className="relative bg-white rounded-2xl shadow-sm border border-gray-200 p-1 flex-1 h-full max-h-[600px] flex flex-col">
                    ...
                    <div className="flex-grow relative rounded-xl overflow-hidden">
                        <D3Scatter
                            data={foodData}
                            xAccessor={xFn} yAccessor={yFn} colorAccessor={colorNutri}
                            selectedIndices={selectedIndices} onSelectionChange={setSelectedIndices}
                            onHoverChange={handleHover} interactionMode={interactionMode}
                            zoomTransform={globalTransform} onZoomChange={setGlobalTransform}
                            width={600} height={550} // Adjust sizes as needed or make responsive
                        />
                    </div>
                </div>
                ...
                <div className="relative bg-white rounded-2xl shadow-sm border border-gray-200 p-1 flex-1 h-full max-h-[600px] flex flex-col">
                    ...
                    <div className="flex-grow relative rounded-xl overflow-hidden">
                        <D3Scatter
                            data={foodData}
                            xAccessor={xFn} yAccessor={yFn} colorAccessor={colorNova}
                            selectedIndices={selectedIndices} onSelectionChange={setSelectedIndices}
                            onHoverChange={handleHover} interactionMode={interactionMode}
                            zoomTransform={globalTransform} onZoomChange={setGlobalTransform}
            ...
            {/* Hover Tooltip */}
            {hoverInfo && (
                <div className="fixed z-50 bg-white/95 backdrop-blur p-4 rounded-lg shadow-2xl border border-gray-100 pointer-events-none text-sm min-w-[200px]"
                     style={{ left: hoverInfo.x + 20, top: hoverInfo.y + 20 }}>
                    <div className="font-bold text-gray-900 text-base mb-1">{hoverInfo.data.name}</div>
                    <div className="text-gray-500 text-xs uppercase tracking-wide border-b border-gray-100 pb-2 mb-2">{hoverInfo.data.brand}</div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                        <span className="text-gray-500">Nutri-Score: <b className="text-gray-900">{hoverInfo.data.nutriscore.toUpperCase()}</b></span>
